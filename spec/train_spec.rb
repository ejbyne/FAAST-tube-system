...
describe Train do
  ...
  let (:train) { Train.new(initial_station: :old_street, carriage_capacity: 7) }
  let (:default_train) { default_train = Train.new }
  ...
  it "allows an initial station to be set" do
    expect(train.current_station).to be(:old_street) #need class instance?
